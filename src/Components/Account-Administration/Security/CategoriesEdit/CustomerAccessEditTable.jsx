...
export default function CustomerAccessEditTable() {
  ...
  const { data: customers, loading } = useFetch("customer-management");
  ...
  if (customers !== null && customers.length > 1) { var isCheckAll = checkedItems.length === customers.length; }
  ...
  const getCheckedStatus = (itemId) => {
    const found = checkedItems.find((item) => item.id === itemId && item.status==1);
    ...
    <div>
      <table className="table table-light table-hover table-striped table table-bordered" >
        ...
        <tbody className="table-group-divider">
          {loading ? <span>Loading</span> : customers?.map((d) => {
